		...
		>
			{!isSender && isFirstInSequence && (
				<Image
					source={{
						uri:
							messageUsers[message.senderId]?.avatarURL ||
			...
			{!isSender && !isFirstInSequence && <View className="w-10" />}
						...
						{message.repliedToId && repliedToMessage && (
							<View className="mb-1">
								<ReplyPreview
									...
									isSender={isSender}
									userName={
										messageUsers[repliedToMessage.senderId]
											?.name || "Người dùng"
									}
								/>
							</View>
